...
function Request(options, callback) {
  ...
  this.host     = options.host || 'cloud.mongodb.com';
...
Request.prototype.performRequest = function performRequest() {
  ...
  req = (this.protocol === 'https') ?
        https.request(requestOptions, this.handleResponse.bind(this)) :
        http.request(requestOptions, this.handleResponse.bind(this))
...
Request.prototype.getRequestOptions = function getRequestOptions(defaultOptions) {
  var requestOptions = {
    agent: (this.protocol === 'https') ?
           new https.Agent({ maxSockets: Number(5000) }) :
           new http.Agent({ maxSockets: Number(5000) }),
    host : this.host,
    port : this.port,
